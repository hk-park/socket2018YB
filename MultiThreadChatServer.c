...
struct client_user lu[MAX_CLIENT];
...
void *do_chat(void *arg)
{
    ...
    while(1) {
		...
		printf("massage - %s\n",str);
		for(i=0; i<MAX_CLIENT; i++){
			if(lu[i].user_socket != INVALID_SOCK && str != NULL && !strcmp(lu[i].user_name, str)) {
					printf("whisper, from %s to %s.\n", lu[i].user_name, str);
					write(lu[i].user_socket, chatData, n);
			} else if (lu[i].user_socket != INVALID_SOCK && str == NULL) {
				write(lu[i].user_socket, chatData, n);
			}
		}
...
//return the index of list_c which c_socket is added.
	...
	for (i = 0; i < MAX_CLIENT; i++) {
		...
		if(lu[i].user_socket == INVALID_SOCK) {
			lu[i].user_socket = c_socket;
			if ((n = read(c_socket, user_name, sizeof(user_name))) > 0) {
				strcpy(lu[i].user_name, user_name);
			}
			printf("Connected socket number is %d and name is %s\n",
					lu[i].user_socket, lu[i].user_name);
...
int popClient(int c_socket)
{
    ...
    for (i = 0; i < MAX_CLIENT; i++) {
	pthread_mutex_lock(&mutex);
    	if ( c_socket == lu[i].user_socket ) {
    		lu[i].user_socket = INVALID_SOCK;
			pthread_mutex_unlock(&mutex);
			break;
		}
		pthread_mutex_unlock(&mutex);
