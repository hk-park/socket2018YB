...
void * do_send_chat(void *arg)
{
    ...
    while(1) {
        ...
        if((n = read(0, buf, sizeof(buf))) > 0 ) { //키보드에서 입력받은 문자열을 buf에 저장. read()함수의 첫번째 인자는 file descriptor로써,
            sprintf(chatData, "[%s] %s", nickname, buf);
            write(c_socket, chatData, strlen(chatData)); //서버로 채팅 메시지 전달
            if(!strncmp(buf, escape, strlen(escape))) { //'exit' 메시지 입력하면,
...
void *do_receive_chat(void *arg)
{
    ...
    while(1) {
        ...
        if((n = read(c_socket, chatData, sizeof(chatData))) > 0 ) {
            write(1, chatData, n); //catData를 화면에 출력함 (1=stdout(모니터))
        }
    }
}
