...
int main()
{
	int fd2[2],fd[2]; // pipe로 사용할 파일디스크립터
	char buf[55];
	int pid;
	
	if(pipe(fd) <0 || pipe(fd2)<0) //파이프 생성 실패시 프로그램종료
	{
		printf("[ERROR] 파이프 에러 \n");
		exit(0);
	}
	...
	{ //자식 프로세스
	  ...
	  memset(buf,0x00, 255);
	  read(fd2[0], buf, sizeof(buf)); //read(읽기)때는 fd[0]를 사용 sizeof사용
	  printf("[CHILD] child message : %s\n",buf);	 
	}
	else
	{
